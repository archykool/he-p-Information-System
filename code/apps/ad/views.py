...
def ad_detail(request, id):
    ...
    context = {'ad': img}
    content_type = {'ad': ad }
    return render(request, 'ad/detail.html', locals())
...
def ad_create(request):
    ...
    if request.method == "POST":
        ...
        if ad_post_form.is_valid():
            # 保存数据，但暂时不提交到数据库中
            new_ad = ad_post_form.save(commit=False)
            # 指定当前用户为作者
    ...
    # 如果用户请求获取数据
        ...
        # 赋值上下文
        context = {'ad_post_form': ad_post_form}
        # 返回模板
        return render(request, 'ad/create.html', locals())
...
def ad_update(request, id):
    ...
    if request.method == "POST":
        ...
        if ad_post_form.is_valid():
            ...
            ad.body = request.POST['body']
            ad.save()
            # 完成后返回到修改后的帖子中。需传入帖子的 id 值
            return redirect("ad:ad_detail", id=id)
        # 如果数据不合法，返回错误信息
        else:
            return HttpResponse("表单内容有误，请重新填写。")
    ...
    # 如果用户 GET 请求获取数据
        ...
        # 将响应返回到模板中
        return render(request, 'ad/update.html', context)
